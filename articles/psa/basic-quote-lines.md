...
description: Овај чланак пружа информације о понудама и редовима понуде.
author: rumant
ms.custom:
...
ms.locfileid: "8933372"
---
# <a name="quotes-and-quote-lines"></a>Понуде и ставке понуда

[!include [banner](../includes/psa-now-project-operations.md)]

[!INCLUDE[cc-applies-to-psa-app-3.x](../includes/cc-applies-to-psa-app-3x.md)]

У апликацији Dynamics 365 Project Service Automation постоје две врсте понуда: понуде за пројекте и понуде за продају. Ова два типа се разликују на следеће начине:

- У понуди за продају постоји само једна мрежа за ставке. У понуди за пројекат постоје две мреже за ставке: једна за ставке пројекта и једна за ставке производа.
- Понуда за продају подржава активацију и ревизије. Понуда за пројекат не подржава те процесе.
- Понуди за продају можете приложити више поруџбина. У понуду за пројекат можете да приложите само један уговор о пројекту.
- Можете да вам буде одобрена понуда за продају и да повезана могућност за пословање остане отворена. Када вам одобре понуду за пројекат, биће затворена повезана могућност за пословање.
- Понуда за продају не садржи нека поља и концепте који су обухваћени у понуди за пројекат. У поља спадају **Уговорна јединица**, **Менаџер за пословне контакте** и **Име контакта за фактурисање**.  
- Понуде за продају и понуде за пројекте идентификује и поље засновано на скупу опција под називом **Тип**. За понуду за продају, ово поље има вредност **Засновано на ставци**. За понуду за пројекат, има вредност **Засновано на раду**.

Овај чланак ће бити фокусиран на детаље пројектне понуде.

Понуда за пројекат у апликацији PSA може да има више ставки предмета или ставки понуде. У ствари, понуда за пројекат има две мреже за ставке предмета. Једна мрежа је намењена ставкама заснованим на пројекту и омогућава детаљне процене. Друга мрежа је намењена ставкама заснованим на производу и користи једноставну јединичну цену и приступ заснован на количини.

- **Засновано на пројекту** – износ (понуђена вредност) утврђује се након процене количине посла. Можете да процените посао на вишем нивоу или да га процените директно као детаље ставке испод сваке ставке понуде. На крају, можете проценити посао на основу почетних процена, коришћењем пројекта и плана пројекта. Ставке понуде засноване на пројекту се налазе само у понудама заснованим на пројекту које се креирају помоћу апликације Project Service Automation. Овај тип ставке понуде је прилагођени образац ручно додатих ставки понуде које су доступне у систему Microsoft Dynamics 365 Sales.
- **Засновано на производу** – износ (понуђена вредност) одређује се на основу количине продатих јединица и продајне цене по јединици производа. Производ у ставци заснованој на производу може доћи из каталога производа у Продаји или може бити производ који дефинишете. Овај тип ставке понуде је такође доступан у понудама заснованим на пројекту које се креирају помоћу апликације PSA.

Износ у понуди је збирна вредност свих ставки заснованих на производу и ставки заснованих на пројекту.

> [!NOTE]
> Понуде и ставке понуда нису потребне у апликацији PSA. Процес пројекта можете започети уговором о пројекту (продат пројекат). Међутим, увек је потребна могућност за пословање, без обзира на то да ли сте започели понудом или уговором о пројекту.

## <a name="project-based-quote-lines"></a>Ставке понуда засноване на пројекту

Ставка понуде заснована на пројекту у апликацији PSA има следеће начине наплате:
...
PSA подржава све три врсте распореда фактурисања за ставке понуде са фиксном ценом.

## <a name="transaction-classification"></a>Класификација трансакције

Организације које пружају професионалне услуге обично дају понуде клијентима и фактуришу клијентима по класификацији трошкова. У апликацији PSA, трошкови су представљени следећим класификацијама трансакција:
...
- **Контролна тачка** – ову класификацију користи логика наплате фиксне цене у апликацији PSA.
...
> [!NOTE]
> Комбинација класификација пројекта и трансакције **Време**, **Трошкови** и **Накнада** повезана са ставком понуде или предметом уговора мора бити јединствена. Ако је иста комбинација класе пројекта и трансакције повезана са више од једног предмета уговора или ставки понуде, PSA неће радити исправно.

## <a name="billing-types"></a>Типови наплате

Поље **Врста наплате** дефинише концепт наплативости у апликацији PSA. То је скуп опција који има следеће могуће вредности:
...
Распоред за фактурисање представља низ датума када се фактуришу трошкови за пројекат. Опционално можете да креирате распоред за фактурисање у ставци понуде у апликацији PSA. Свака ставка понуде може имати свој распоред за фактурисање. Да бисте креирали распоред за фактурисање, морате обезбедити следеће вредности атрибута:

- Датум почетка наплате 
- Датум испоруке који представља датум завршетка наплате за пројекат
- Учесталост фактурисања

PSA користи ове три вредности атрибута да би генерисао условни скуп датума за успостављање фактурисања.
