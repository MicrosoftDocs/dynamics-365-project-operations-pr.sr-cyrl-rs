...
- **Структурна анализа посла**: Структурна анализа посла у предлошку пројекта има исти скуп елемената као и пројекат. Можете да креирате хијерархију задатака, повежете улоге са задатком, дефинишете атрибуте распореда, поставите зависности и прикажете све податке у ганту. Структура анализе рада у предлошцима пројеката такође подржава режиме задатака за сваки задатак. Приликом креирања радног распореда не постоји разлика између предлошка пројекта и пројекта.  
...
## <a name="create-a-project-template-from-an-existing-project"></a>Креирање предлошка пројекта од постојећег пројекта
Предложак пројекта можете креирати од пројекта на следеће начине:

- **Структура анализе рада** : Структура анализе рада у предлошку који је изведен из пројекта копираће све задатке и зависности. Креиране доделе ће бити засноване на генеричким члановима тима који се додају пројектском тиму када се креира предложак пројекта.
- **Процене пројекта** : Када се предложак пројекта креира из постојећег пројекта, процене из изворног пројекта се копирају у предложак пројекта.
- **Чланови пројектног** тима: Када се предложак креира из постојећег пројекта, сви именовани чланови тима биће замењени генеричким ресурсом организације. Одржавају се сва имена позиција и улоге.

## <a name="create-a-project-from-a-template"></a>Креирање пројекта из предлошка  
 Пројекат можете да креирате од предлошка на следеће начине:  
