...
description: Овај чланак садржи објашњења о томе како да креирате структуру анализе рада (WБС) укључујући основне контроле у новом интерфејсу за планирање.
author: ruhercul
ms.date: 12/16/2021
ms.topic: article
...
Задаци се могу преместити на било који ниво у структури радне анализе на један од два начина:
...
Атрибутима ангажовања се приступа у пољу **Ресурси** у распореду. Можете потражити постојећи ресурс или изаберите **Креирај** и у окну **Брзо креирање** додајте члана пројектног тима као нови ресурс.  Када тражите ресурс помоћу издвајања ресурса у координатној мрежи задатака, приказу табле или ганту, претрага враћа постојеће чланове пројектног тима или активне ресурсе који се могу резервисати.
...
Поља **Улога**, **Јединица за обезбеђивање ресурса** и **Назив позиције** се користе за описивање потреба за ангажовањем ресурса на задатку. Ови атрибути ангажовања, заједно са распоредом задатака, користе се за проналажење доступних ресурса за обављање овог задатка.
   ...
   - **Улога** : Наведите тип ресурса који је потребан за задатак.,
...
## <a name="understanding-the-impacts-of-duration-resource-calendars-and-project-calendars-on-tasks"></a>Разумевање утицаја трајања, календара ресурса и пројектног календара на задатке
Трајање задатка се дефинише као број радних сати између времена почетка датума почетка и времена завршетка задатка.   Пројекат за wеб дефинише јединице мере трајања на следећи начин:

| **Мера трајања** | **Количина**|
|----------------------------------------------------|----------------------|
| Часови дневно | 8 |
| Часови недељно |  40 |
| Дани месечно |  20 |

Недодерени задаци се заказали помоћу календара пројекта. Међутим, након почетног додељивања ресурса, планирање задатка се ажурира тако да поштује календар ресурса. Накнадне промене задатка који има задатак биће регулисане [режимом заказивања](scheduling-modes.md) пројекта. Да бисте сазнали више о утицају календара на задатке, погледајте [календаре ресурса у пројекту за веб и](https://techcommunity.microsoft.com/t5/project-blog/resource-calendars-in-project-for-the-web/ba-p/3269686)[Таск Старт Тимес & Yоур Пројецтс!](https://techcommunity.microsoft.com/t5/project-blog/task-start-times-amp-your-projects/ba-p/3269665)
...
| Максималне ставке контролне листе по задатку                   | 20                   |
